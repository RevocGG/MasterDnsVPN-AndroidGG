...
type Stats struct {
	...
	// CheckedResolverCount: probes FINISHED (valid + rejected) — scan progress.
...
func GetStats(key string) *Stats {
	...
	return &Stats{
		...
		CheckedResolverCount: s.CheckedResolverCount,
		ListenAddr:           s.ListenAddr,
	}
}

// GetResolverQualityCSV returns the live per-resolver quality snapshot for a
// running instance as CSV lines "resolver,port,valid,sent,acked,lost,avgRTTms"
// (empty string when the instance is not running). CSV keeps the gomobile
// binding trivially supported (no slices across the JNI boundary).
func GetResolverQualityCSV(key string) string {
	raw := mobile.GetResolverQualitySnapshot(key)
	if len(raw) == 0 {
		return ""
	}
	...
	for i, q := range raw {
		...
		fmt.Fprintf(&b, "%s,%d,%t,%d,%d,%d,%.1f", q.Resolver, q.Port, q.IsValid, q.Sent, q.Acked, q.Lost, q.AvgRTTms)
	}
	return b.String()
}

// UpdateBestMatchList refreshes (throttled to 30s) the Best Match resolver
// file of a running instance from live quality stats. Returns the number of
// resolvers written, or -1 when throttled.
func UpdateBestMatchList(key string) int64 {
	n, err := mobile.UpdateBestMatchList(key)
	if err != nil {
		return 0
	}
	return int64(n)
...
// ReadBestMatchListText returns the persisted Best Match resolver file body
// for a profile directory (empty when none saved yet).
func ReadBestMatchListText(profileDir string) string {
	lines := mobile.ReadBestMatchList(profileDir)
	return strings.Join(lines, "\n")
}

// UpdateBestMatchGlobal aggregates live resolver quality across ALL running
// instances, persists the shared Best Match list (throttled) and returns the
// ranked snapshot as CSV lines "addr,scorePct" (empty when nothing eligible).
func UpdateBestMatchGlobal(filesDir string) string {
	return mobile.UpdateBestMatchGlobal(filesDir)
}

// ReadBestMatchGlobalText returns the persisted shared Best Match resolver
// lines (newline-joined) for the app filesDir (empty when none saved yet).
func ReadBestMatchGlobalText(filesDir string) string {
	lines := mobile.ReadBestMatchGlobal(filesDir)
	return strings.Join(lines, "\n")
}
...
// instanceID is the key passed to StartInstance (used to resolve the local
// SOCKS5 credentials when the profile enables SOCKS5 authentication);
...
func StartTunBridge(instanceID string, tunFd int32, mtu int32, listenAddr string) error {
	return mobile.StartTunBridge(instanceID, tunFd, mtu, listenAddr)
}
...
// SetTunDisableIPv6 toggles the TUN bridge-wide "Disable IPv6" behaviour:
// AAAA DNS-answer filtering plus fast RST of IPv6 TCP CONNECTs. Defaults to
// true — the DNS tunnel has no IPv6 egress, so IPv6 attempts only produce
// "SOCKS5 connect refused code 3" timeouts in apps like YouTube.
func SetTunDisableIPv6(disabled bool) {
	mobile.SetTunDisableIPv6(disabled)
}

// GetTunDisableIPv6 reports the current "Disable IPv6" state.
func GetTunDisableIPv6() bool {
	return mobile.GetTunDisableIPv6()
}
